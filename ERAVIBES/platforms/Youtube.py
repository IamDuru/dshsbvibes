...
class YouTubeAPI:
    ...
    async def _download(
        ...
        if videoid:
            link = self.base + link
        # Agar song video download karna ho toh purana method use karte hain
        if songvideo:
            def song_video_dl():
                ...
                x.download([link])
            await asyncio.get_running_loop().run_in_executor(None, song_video_dl)
            ...
            def song_audio_dl():
                ...
                x = yt_dlp.YoutubeDL(ydl_optssx)
                x.download([link])
            await asyncio.get_running_loop().run_in_executor(None, song_audio_dl)
            fpath = f"downloads/{title}.mp3"
            return fpath

        elif video:
            # Ab hum video ke liye direct streaming URL le rahe hain,
            # jisse file download karne ka time na lage.
            proc = await asyncio.create_subprocess_exec(
                "yt-dlp",
                "-g",
                "-f",
            ...
            # Audio ke liye direct streaming URL
            proc = await asyncio.create_subprocess_exec(
                "yt-dlp",
                "-g",
                "-f",
